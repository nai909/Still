...
export default function ZenGardenMode() {
  ...
  }, [isStarted, initAudio, mkBowlTone]);
  ...
  useEffect(() => {
    ...
    sceneRef.current.background = new THREE.Color(0x8aaccc);
    sceneRef.current.fog = new THREE.FogExp2(0x8aaccc, 0.008);
    ...
    // === LIGHTS ===
    ambLightRef.current = new THREE.AmbientLight(0x556677, 0.45);
    sceneRef.current.add(ambLightRef.current);

    sunLightRef.current = new THREE.DirectionalLight(0xffeedd, 1.0);
    sunLightRef.current.position.set(-10, 25, 5);
    ...
    fillLightRef.current = new THREE.DirectionalLight(0x99aabb, 0.3);
    ...
    hemiLightRef.current = new THREE.HemisphereLight(0xaabbcc, 0x554433, 0.3);
    ...
    lanternLightRef.current = new THREE.PointLight(0xffaa55, 0.1, 6, 2);
    ...
    particlesRef.current = new THREE.Points(ffGeo, new THREE.PointsMaterial({
      color: 0xffdd88, size: 0.15, transparent: true, opacity: 0.0, sizeAttenuation: true
    }));
    ...
    const animate = () => {
      ...
      if (particlesRef.current) {
        ...
        const ffBase = todCurrentRef.current.firefly || 0;
      ...
      if (lanternLightRef.current) {
        const lBase = todCurrentRef.current.lantern || 0.1;
  ...
  }, [isStarted, rakeAt, rakeSndOn, rakeSndOff]);
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#0a0a08' }}>
      ...
      {!isStarted && (
        ...
        >
          <h1 style={{
            fontWeight: 300, fontSize: 'clamp(32px, 6vw, 72px)',
            color: 'rgba(200, 180, 150, 0.7)', letterSpacing: '0.3em', marginBottom: '0.5em',
            fontFamily: '"Jost", sans-serif'
          }}>
            zen garden
          </h1>
          <p style={{
            fontWeight: 300, fontSize: 'clamp(12px, 1.8vw, 18px)',
            color: 'rgba(200, 180, 150, 0.3)', letterSpacing: '0.15em',
            fontFamily: '"Jost", sans-serif',
            animation: 'breathe 4s ease-in-out infinite'
          }}>
            tap to enter stillness
          </p>
        </div>
      )}
      ...
      <div style={{
        ...
        opacity: isStarted ? 1 : 0, transition: 'opacity 1.5s ease 2s',
