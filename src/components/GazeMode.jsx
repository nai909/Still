...
const THREE_D_MODES = new Set([
  ...
  'machinaTemporis', 'oceanusProfundus', 'portaDimensionum',
  'fungusDimensio'
]);
...
function GazeMode({ theme, primaryHue = 162, onHueChange, backgroundMode = false, currentVisual, onVisualChange, breathSession = null, externalTouchRef = null }) {
  ...
  React.useEffect(() => {
    ...
    // Initialize - fewer jellyfish, larger and more majestic, starting from center
    const jellies = [];
    const jellyCount = Math.min(Math.floor(canvas.width / 400) + 1, 3);
    for (let i = 0; i < jellyCount; i++) {
      // Spread widely around center based on count
      const offsetX = (i - (jellyCount - 1) / 2) * 200;
      const offsetY = (i - (jellyCount - 1) / 2) * 150;
  ...
  }, [currentMode, getInteractionInfluence]);
