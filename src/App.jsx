...
const gazeModes = [
  ...
  { key: 'candleFlame', name: 'Candle' },
  { key: 'incenseSmoke', name: 'Incense' },
  { key: 'hourglass', name: 'Hourglass' },
  { key: 'cymatics', name: 'Cymatics' },
  { key: 'inkWater', name: 'Ink' },
  { key: 'mycelium', name: 'Mycelium' },
  { key: 'treeLungs', name: 'Tree Lungs' },
  { key: 'sandMandala', name: 'Mandala' },
...
function GazeMode({ theme, primaryHue = 162, onHueChange, backgroundMode = false, currentVisual, onVisualChange, breathSession = null, externalTouchRef = null }) {
  ...
  // ========== CANDLE FLAME MODE ==========
  React.useEffect(() => {
    if (currentMode !== 'candleFlame' || !containerRef.current || typeof THREE === 'undefined') return;
    ...
    // Candle body (simple cylinder)
    const candleGeom = new THREE.CylinderGeometry(0.15, 0.15, 1.5, 16);
    const candleMat = new THREE.MeshBasicMaterial({ color: 0x2a2a3a, wireframe: true, transparent: true, opacity: 0.3 });
    const candle = new THREE.Mesh(candleGeom, candleMat);
    candle.position.y = -1;
    scene.add(candle);

    // Wick
    const wickGeom = new THREE.CylinderGeometry(0.01, 0.01, 0.15, 8);
    const wickMat = new THREE.MeshBasicMaterial({ color: 0x333333 });
    const wick = new THREE.Mesh(wickGeom, wickMat);
    wick.position.y = -0.18;
    scene.add(wick);

    // Flame particles
    const particleCount = Math.floor(80 * MOBILE_PARTICLES);
    const particles = [];
    ...
    // Outer glow
    const outerGlowGeom = new THREE.SphereGeometry(0.4, 16, 16);
    const hslToHex = (h, s, l) => {
      s /= 100; l /= 100;
      const a = s * Math.min(l, 1 - l);
      const f = n => { const k = (n + h / 30) % 12; return l - a * Math.max(Math.min(k - 3, 9 - k, 1), -1); };
    ...
    const outerGlowMat = new THREE.MeshBasicMaterial({ color: hslToHex(hue, 60, 50), transparent: true, opacity: 0.15 });
    const outerGlow = new THREE.Mesh(outerGlowGeom, outerGlowMat);
    outerGlow.position.y = 0.2;
    scene.add(outerGlow);
    ...
    const animate = () => {
      ...
      // Flame height based on breath (grows on inhale)
      const flameHeight = 0.5 + breath * 0.8;
      const flameIntensity = 0.6 + breath * 0.4;

      // Touch interaction - flame leans away from touch
      let targetLean = Math.sin(elapsed * 2) * 0.1;
      if (touchPointsRef.current.length > 0) {
        const touch = touchPointsRef.current.find(p => p.active) || touchPointsRef.current[0];
        if (touch) {
          const normalizedX = (touch.x / window.innerWidth - 0.5) * 2;
          targetLean = -normalizedX * 0.4;
        }
      }
      const force = (targetLean - flameLean) * springStiffness;
      leanVelocity = (leanVelocity + force) * springDamping;
      flameLean += leanVelocity;
      ...
      particles.forEach((p, i) => {
        ...
        // Rise from wick
        p.position.y = t * data.maxHeight * flameHeight;

        // Wobble and lean
        const wobble = Math.sin(elapsed * 5 + i) * data.wobble * (1 - t);
        p.position.x = wobble + flameLean * (1 - t * 0.5);
        p.position.z = Math.cos(elapsed * 3 + i * 0.5) * data.wobble * 0.3;

        // Fade and shrink as rises
        p.material.opacity = (1 - t) * flameIntensity;
        const scale = (1 - t * 0.7);
        p.scale.setScalar(scale);

        // Color shift from yellow core to orange/red tips
        const colorT = t;
        const r = 1;
        const g = 0.7 - colorT * 0.4;
        const b = 0.2 - colorT * 0.2;
        p.material.color.setRGB(r, g, Math.max(0, b));
      });

      // Glow pulse
      glow.scale.setScalar(1 + breath * 0.3 + Math.sin(elapsed * 8) * 0.05);
      glow.material.opacity = 0.5 + breath * 0.3;
      outerGlow.scale.setScalar(1 + breath * 0.5);
      outerGlow.material.opacity = 0.1 + breath * 0.1;
  ...
  // ========== INCENSE SMOKE MODE ==========
  React.useEffect(() => {
    if (currentMode !== 'incenseSmoke' || !containerRef.current || typeof THREE === 'undefined') return;
    ...
    // Incense stick
    const stickGeom = new THREE.CylinderGeometry(0.02, 0.02, 2, 8);
    const stickMat = new THREE.MeshBasicMaterial({ color: 0x4a3728, wireframe: true, transparent: true, opacity: 0.4 });
    ...
    // Ember tip
    const emberGeom = new THREE.SphereGeometry(0.04, 8, 8);
    const emberMat = new THREE.MeshBasicMaterial({ color: 0xff4400, transparent: true, opacity: 0.8 });
    const ember = new THREE.Mesh(emberGeom, emberMat);
    ember.position.set(0.1, -0.5, 0);
    scene.add(ember);

    // Smoke particles
    const particleCount = Math.floor(150 * MOBILE_PARTICLES);
    const particles = [];
    const smokeColor = hslToHex(hue, 20, 70);

    for (let i = 0; i < particleCount; i++) {
      const geom = new THREE.SphereGeometry(0.08 + Math.random() * 0.08, 8, 8);
      const mat = new THREE.MeshBasicMaterial({
        color: smokeColor,
        transparent: true,
        opacity: 0
    ...
    const animate = () => {
      ...
      if (touchPointsRef.current.length > 0) {
        const touch = touchPointsRef.current.find(p => p.active) || touchPointsRef.current[0];
        if (touch) {
          touchForceX = (touch.x / window.innerWidth - 0.5) * 0.5;
          touchForceZ = (touch.y / window.innerHeight - 0.5) * 0.3;
        }
      }

      // Exhale billows smoke outward, inhale draws upward
      const billowAmount = 1 - breath; // More spread on exhale
      const riseSpeed = 0.5 + breath * 0.5; // Faster rise on inhale
      ...
      particles.forEach(p => {
        ...
        if (data.life > 1) {
          // Reset particle
          data.life = 0;
          p.position.set(0.1, -0.5, 0);
          data.curl = Math.random() * Math.PI * 2;
        }

        const t = data.life;

        // Rise with curl
        p.position.y = -0.5 + t * 4 * riseSpeed;

        // Curl pattern (perlin-like using sin)
        const curlX = Math.sin(data.curl + elapsed * data.curlSpeed) * (0.3 + billowAmount * 0.5) * t;
        const curlZ = Math.cos(data.curl * 1.3 + elapsed * data.curlSpeed * 0.7) * 0.2 * t;

        p.position.x = 0.1 + curlX + data.drift * t + touchForceX * t;
        p.position.z = curlZ + touchForceZ * t;

        // Fade in then out
        const fadeIn = Math.min(t * 4, 1);
        const fadeOut = 1 - Math.pow(t, 2);
        p.material.opacity = fadeIn * fadeOut * 0.35;

        // Grow as rises
        const scale = (1 + t * 2) * data.size / 0.08;
        p.scale.setScalar(scale);
      });

      // Ember glow pulse
      ember.material.opacity = 0.6 + Math.sin(elapsed * 3) * 0.2 + breath * 0.2;
  ...
  // ========== HOURGLASS MODE ==========
  React.useEffect(() => {
    if (currentMode !== 'hourglass' || !containerRef.current || typeof THREE === 'undefined') return;
    ...
    const glassColor = hslToHex(hue, 30, 60);

    // Top bulb (cone)
    const topGeom = new THREE.ConeGeometry(1, 1.8, 24, 1, true);
    const glassMat = new THREE.MeshBasicMaterial({ color: glassColor, wireframe: true, transparent: true, opacity: 0.4 });
    const topBulb = new THREE.Mesh(topGeom, glassMat);
    topBulb.position.y = 1.1;
    topBulb.rotation.x = Math.PI;
    hourglassGroup.add(topBulb);

    // Bottom bulb (cone)
    const bottomGeom = new THREE.ConeGeometry(1, 1.8, 24, 1, true);
    const bottomBulb = new THREE.Mesh(bottomGeom, glassMat.clone());
    bottomBulb.position.y = -1.1;
    hourglassGroup.add(bottomBulb);

    // Neck (thin cylinder)
    const neckGeom = new THREE.CylinderGeometry(0.08, 0.08, 0.4, 12);
    const neck = new THREE.Mesh(neckGeom, glassMat.clone());
    hourglassGroup.add(neck);

    // Frame rings
    const ringGeom = new THREE.TorusGeometry(1.05, 0.03, 8, 32);
    const topRing = new THREE.Mesh(ringGeom, glassMat.clone());
    topRing.position.y = 2;
    topRing.rotation.x = Math.PI / 2;
    hourglassGroup.add(topRing);

    const bottomRing = new THREE.Mesh(ringGeom.clone(), glassMat.clone());
    bottomRing.position.y = -2;
    bottomRing.rotation.x = Math.PI / 2;
    hourglassGroup.add(bottomRing);

    scene.add(hourglassGroup);

    // Sand particles
    const sandCount = Math.floor(200 * MOBILE_PARTICLES);
    const sandParticles = [];
    const sandColor = 0xd4a84b;

    for (let i = 0; i < sandCount; i++) {
      const geom = new THREE.SphereGeometry(0.025, 4, 4);
      const mat = new THREE.MeshBasicMaterial({ color: sandColor, transparent: true, opacity: 0.8 });
      const grain = new THREE.Mesh(geom, mat);

      // Start in top or bottom based on initial distribution
      const inTop = Math.random() > 0.3;
      grain.userData = {
        inTop,
        falling: false,
        fallProgress: 0,
        restY: inTop ? 0.5 + Math.random() * 1.2 : -1.8 + Math.random() * 0.8,
        restX: (Math.random() - 0.5) * 0.6,
        restZ: (Math.random() - 0.5) * 0.6,
        fallDelay: Math.random() * 2
      };

      sandParticles.push(grain);
      hourglassGroup.add(grain);
    }
    ...
    const animate = () => {
      ...
      if (touchPointsRef.current.length > 0) {
        const touch = touchPointsRef.current.find(p => p.active) || touchPointsRef.current[0];
        if (touch) {
          targetTilt = (touch.x / window.innerWidth - 0.5) * 0.5;
        }
      }
      const force = (targetTilt - currentTilt) * springStiffness;
      tiltVelocity = (tiltVelocity + force) * springDamping;
      currentTilt += tiltVelocity;
      hourglassGroup.rotation.z = currentTilt;

      // Slow rotation
      hourglassGroup.rotation.y += 0.001;

      // Sand behavior: falls on exhale, rises on inhale (magical)
      const falling = breath < 0.5; // Exhale = sand falls

      sandParticles.forEach((grain, i) => {
        const data = grain.userData;

        if (falling && data.inTop && elapsed > data.fallDelay) {
          // Start falling
          data.falling = true;
        }

        if (!falling && !data.inTop) {
          // Magic rise on inhale
          data.falling = false;
          data.inTop = true;
          data.fallProgress = 0;
          data.restY = 0.5 + Math.random() * 1.2;
          data.fallDelay = elapsed + Math.random() * 2;
        }

        if (data.falling) {
          data.fallProgress += 0.008;
          if (data.fallProgress > 1) {
            data.inTop = false;
            data.falling = false;
            data.restY = -1.8 + Math.random() * 0.8;
          }
        }
        ...
        if (data.falling) {
          ...
          // Fall through neck
          const narrowing = Math.sin(t * Math.PI);
          grain.position.x = data.restX * (1 - narrowing * 0.9);
          grain.position.z = data.restZ * (1 - narrowing * 0.9);
          grain.position.y = THREE.MathUtils.lerp(data.restY, -1.8 + Math.random() * 0.5, t);
        } else {
          // Settle in current bulb
          const targetY = data.restY;
          grain.position.y = THREE.MathUtils.lerp(grain.position.y, targetY, 0.05);

          // Constrain to cone shape
          const distFromCenter = Math.abs(grain.position.y - (data.inTop ? 1.1 : -1.1));
          const maxRadius = Math.max(0.1, (1 - distFromCenter / 1.8) * 0.8);
          grain.position.x = data.restX * maxRadius;
          grain.position.z = data.restZ * maxRadius;
        }

        grain.material.opacity = 0.7 + breath * 0.2;
      });

      // Glass opacity with breath
      glassMat.opacity = 0.3 + breath * 0.2;
  ...
  // ========== CYMATICS MODE ==========
  React.useEffect(() => {
    if (currentMode !== 'cymatics' || !containerRef.current || typeof THREE === 'undefined') return;
    ...
    // High-poly circular plane for wave displacement
    const segments = isMobile ? 64 : 100;
    const geometry = new THREE.CircleGeometry(3, segments, segments);
    const material = new THREE.MeshBasicMaterial({
    ...
    // Outer ring
    const ringGeom = new THREE.TorusGeometry(3.05, 0.05, 8, 64);
    const ring = new THREE.Mesh(ringGeom, material.clone());
    ring.rotation.x = Math.PI / 2;
    scene.add(ring);
    ...
    const animate = () => {
      ...
      // Touch changes frequency
      if (touchPointsRef.current.length > 0) {
        const touch = touchPointsRef.current.find(p => p.active) || touchPointsRef.current[0];
        if (touch) {
          targetFreq = 2 + (touch.y / window.innerHeight) * 8;
        }
      } else {
        targetFreq = 3 + breath * 4; // More complex on inhale
      }
      frequency += (targetFreq - frequency) * 0.05;

      // Wave amplitude based on breath
      const amplitude = 0.1 + breath * 0.3;

      // Update vertex positions for cymatics pattern
      for (let i = 0; i < positions.count; i++) {
        ...
        // Chladni-like pattern: sin(n*angle) * Bessel-like radial
        const radial = Math.sin(distance * frequency + elapsed * 2);
        const angular = Math.cos(angle * Math.floor(frequency));
        const pattern = radial * angular;

        positions.setY(i, originalY[i] + pattern * amplitude);
      }
  ...
  // ========== INK IN WATER MODE ==========
  React.useEffect(() => {
    if (currentMode !== 'inkWater' || !containerRef.current || typeof THREE === 'undefined') return;
    ...
    // Ink blob particles
    const blobCount = Math.floor(40 * MOBILE_PARTICLES);
    const blobs = [];
    const inkColor = hslToHex(hue, 60, 30);
    const edgeColor = hslToHex(hue, 70, 50);
    ...
    for (let i = 0; i < blobCount; i++) {
      ...
      // Edge glow
      const edgeGeom = new THREE.SphereGeometry(size * 1.2, 12, 12);
      ...
      blob.userData = {
        life: -Math.random() * 3, // Staggered start
        maxLife: 5 + Math.random() * 3,
        startX: (Math.random() - 0.5) * 2,
        startY: 1 + Math.random(),
        driftX: (Math.random() - 0.5) * 0.3,
        driftY: -0.2 - Math.random() * 0.2,
        size,
        edge
      };
    ...
    const animate = () => {
      ...
      // Release new ink on exhale
      const exhaling = breath < lastBreath;
      lastBreath = breath;

      // Touch creates new ink drops
      if (touchPointsRef.current.length > 0) {
        const touch = touchPointsRef.current.find(p => p.active);
        if (touch) {
          // Find an inactive blob and activate it at touch position
          const inactive = blobs.find(b => b.userData.life < 0 || b.userData.life > b.userData.maxLife);
          if (inactive) {
            const data = inactive.userData;
            data.life = 0;
            data.startX = (touch.x / window.innerWidth - 0.5) * 5;
            data.startY = -(touch.y / window.innerHeight - 0.5) * 4;
          }
        }
      }

      // Bloom amount based on breath (more spread on exhale)
      const bloomFactor = 1 + (1 - breath) * 0.5;
      ...
      blobs.forEach(blob => {
        ...
        if (data.life < 0) {
          ...
          data.edge.material.opacity = 0;
          return;
        }

        if (data.life > data.maxLife) {
          // Reset
          data.life = -Math.random() * 2;
          data.startX = (Math.random() - 0.5) * 2;
          data.startY = 1 + Math.random();
          return;
        }

        const t = data.life / data.maxLife;

        // Position: drift down and outward
        blob.position.x = data.startX + data.driftX * data.life * bloomFactor;
        blob.position.y = data.startY + data.driftY * data.life;
        blob.position.z = Math.sin(data.life + data.startX) * 0.3;

        // Scale: bloom then fade
        const bloomCurve = Math.sin(t * Math.PI);
        const scale = (1 + bloomCurve * 2) * bloomFactor;
        blob.scale.setScalar(scale);

        // Opacity: fade in then out
        const fadeIn = Math.min(t * 5, 1);
        const fadeOut = 1 - Math.pow(t, 1.5);
        blob.material.opacity = fadeIn * fadeOut * 0.6;
        data.edge.material.opacity = fadeIn * fadeOut * 0.2;
      });
  ...
  // ========== TREE LUNGS MODE ==========
  React.useEffect(() => {
    if (currentMode !== 'treeLungs' || !containerRef.current || typeof THREE === 'undefined') return;
    ...
    const treeLungsGroup = new THREE.Group();
    const branchColor = hslToHex(hue, 50, 50);
    const leafColor = hslToHex((hue + 60) % 360, 60, 45); // Greener

    const allBranches = [];
    const allLeaves = [];

    // Create recursive branching structure (lung-like)
    const createLungTree = (startX, startY, mirror = false) => {
      const branches = [];
      const leaves = [];
      ...
      const createBranch = (x, y, angle, length, depth) => {
        ...
        const endX = x + Math.cos(angle) * length * (mirror ? -1 : 1);
        const endY = y + Math.sin(angle) * length;

        const points = [new THREE.Vector3(x, y, 0), new THREE.Vector3(endX, endY, 0)];
        const geom = new THREE.BufferGeometry().setFromPoints(points);
        const mat = new THREE.LineBasicMaterial({
          color: branchColor,
          transparent: true,
          opacity: 0.6 - depth * 0.05
        });
        const line = new THREE.Line(geom, mat);
        line.userData = {
          depth,
        ...
        branches.push(line);
        treeLungsGroup.add(line);

        // Leaves at branch tips
        if (depth > 3) {
          const leafGeom = new THREE.SphereGeometry(0.08 - depth * 0.01, 6, 6);
          const leafMat = new THREE.MeshBasicMaterial({
            color: leafColor,
            transparent: true,
            opacity: 0.4
          ...
          const leaf = new THREE.Mesh(leafGeom, leafMat);
          leaf.position.set(endX, endY, 0);
          leaf.userData = { depth };
          leaves.push(leaf);
          treeLungsGroup.add(leaf);
        }

        // Branch further
        const spread = 0.4 + depth * 0.1;
        const lengthRatio = 0.72;
        createBranch(endX, endY, angle + spread, length * lengthRatio, depth + 1);
        createBranch(endX, endY, angle - spread * 0.6, length * lengthRatio, depth + 1);
        if (depth < 3) {
          createBranch(endX, endY, angle, length * lengthRatio * 0.8, depth + 1);
        }
      };

      // Main bronchus
      createBranch(startX, startY, Math.PI / 2 + (mirror ? 0.3 : -0.3), 1.5, 0);

      return { branches, leaves };
    };

    // Left lung (tree)
    const left = createLungTree(-1.5, -2, true);
    allBranches.push(...left.branches);
    allLeaves.push(...left.leaves);

    // Right lung (tree)
    const right = createLungTree(1.5, -2, false);
    allBranches.push(...right.branches);
    allLeaves.push(...right.leaves);

    // Trachea
    const tracheaPoints = [new THREE.Vector3(0, -2, 0), new THREE.Vector3(0, -3.5, 0)];
    const tracheaGeom = new THREE.BufferGeometry().setFromPoints(tracheaPoints);
    const tracheaMat = new THREE.LineBasicMaterial({ color: branchColor, transparent: true, opacity: 0.7 });
    const trachea = new THREE.Line(tracheaGeom, tracheaMat);
    treeLungsGroup.add(trachea);

    scene.add(treeLungsGroup);
    ...
    const animate = () => {
      ...
      // Branches extend/retract with breath
      allBranches.forEach(branch => {
        const data = branch.userData;
        const breathScale = 0.85 + breath * 0.3;
        const newLength = data.baseLength * breathScale;

        const endX = data.startX + Math.cos(data.angle) * newLength * (data.mirror ? -1 : 1);
        const endY = data.startY + Math.sin(data.angle) * newLength;

        const positions = branch.geometry.attributes.position;
        positions.setXYZ(1, endX, endY, 0);
        positions.needsUpdate = true;

        branch.material.opacity = (0.4 + breath * 0.3) - data.depth * 0.04;
      });

      // Leaves pulse with breath
      allLeaves.forEach(leaf => {
        const scale = 0.8 + breath * 0.6;
        leaf.scale.setScalar(scale);
        leaf.material.opacity = 0.2 + breath * 0.5;
      });
      ...
      if (touchPointsRef.current.length > 0) {
        ...
        if (touch) {
          const normalizedX = (touch.x / window.innerWidth - 0.5) * 2;
          const normalizedY = (touch.y / window.innerHeight - 0.5) * 2;
          treeLungsGroup.rotation.y += normalizedX * 0.01;
          treeLungsGroup.rotation.x += normalizedY * 0.01;
        }
      } else {
        treeLungsGroup.rotation.y += 0.001;
      }
  ...
  // ========== SAND MANDALA MODE ==========
  React.useEffect(() => {
    if (currentMode !== 'sandMandala' || !containerRef.current || typeof THREE === 'undefined') return;
    ...
    const grains = [];
    const grainCount = Math.floor(1500 * MOBILE_PARTICLES);

    // Create sand grains in mandala pattern
    const rings = 12;
    const grainsPerRing = Math.floor(grainCount / rings);

    for (let r = 0; r < rings; r++) {
      const radius = 0.3 + r * 0.22;
      const ringHue = (hue + r * 25) % 360;
      const color = hslToHex(ringHue, 65, 55);

      for (let i = 0; i < grainsPerRing; i++) {
        const angle = (i / grainsPerRing) * Math.PI * 2;
        ...
        // Target position in mandala
        const targetX = Math.cos(angle) * radius;
        const targetY = Math.sin(angle) * radius;

        // Start scattered
        grain.position.x = (Math.random() - 0.5) * 8;
        grain.position.y = (Math.random() - 0.5) * 8;
        grain.position.z = Math.random() * 0.1;
    ...
    const animate = () => {
      ...
      // Build on inhale, scatter on exhale
      const building = breath > 0.3;
      if (building) {
        buildProgress = Math.min(1, buildProgress + delta * 0.15);
      } else {
        buildProgress = Math.max(0, buildProgress - delta * 0.3);
      }

      // Touch accelerates building in that area
      let touchX = 0, touchY = 0, touchActive = false;
      if (touchPointsRef.current.length > 0) {
        const touch = touchPointsRef.current.find(p => p.active);
        if (touch) {
          touchX = (touch.x / window.innerWidth - 0.5) * 6;
          touchY = -(touch.y / window.innerHeight - 0.5) * 6;
          touchActive = true;
        }
      }
      ...
      grains.forEach((grain, i) => {
        const data = grain.userData;
        const ringProgress = Math.max(0, buildProgress - data.ring * 0.06);

        if (ringProgress > 0) {
          // Move toward target
          const touchDist = touchActive
            ? Math.sqrt(Math.pow(data.targetX - touchX, 2) + Math.pow(data.targetY - touchY, 2))
            : 10;
          const touchBoost = touchActive && touchDist < 1.5 ? 2 : 1;
          const speed = 0.05 * touchBoost;

          grain.position.x += (data.targetX - grain.position.x) * speed * ringProgress;
          grain.position.y += (data.targetY - grain.position.y) * speed * ringProgress;
          grain.position.z *= 0.95;

          grain.material.opacity = Math.min(0.9, ringProgress * 1.2);
        } else {
          // Scatter
          grain.position.x += (Math.random() - 0.5) * 0.1;
          grain.position.y += (Math.random() - 0.5) * 0.1;
          grain.material.opacity *= 0.95;
        }
      });

      // Update lines
      lines.forEach(line => {
        line.material.opacity = buildProgress * 0.4;
      });
  ...
  // ========== MURMURATION MODE ==========
  React.useEffect(() => {
    if (currentMode !== 'murmuration' || !containerRef.current || typeof THREE === 'undefined') return;
    ...
    // Boids parameters
    const boidCount = Math.floor(250 * MOBILE_PARTICLES);
    const boids = [];

    // Create bird-like shapes
    for (let i = 0; i < boidCount; i++) {
      const geom = new THREE.ConeGeometry(0.05, 0.15, 4);
      ...
      const boid = new THREE.Mesh(geom, mat);

      // Random initial position in sphere
      const theta = Math.random() * Math.PI * 2;
      const phi = Math.acos(2 * Math.random() - 1);
      const r = 3 + Math.random() * 2;
      boid.position.x = r * Math.sin(phi) * Math.cos(theta);
      boid.position.y = r * Math.sin(phi) * Math.sin(theta);
      boid.position.z = r * Math.cos(phi);
    ...
    const alignment = (boid, neighbors) => {
      const avg = new THREE.Vector3();
      if (neighbors.length === 0) return avg;
      neighbors.forEach(other => avg.add(other.userData.velocity));
      avg.divideScalar(neighbors.length);
      return avg.sub(boid.userData.velocity).multiplyScalar(0.05);
    };

    const cohesion = (boid, neighbors) => {
      const center = new THREE.Vector3();
      if (neighbors.length === 0) return center;
      neighbors.forEach(other => center.add(other.position));
      center.divideScalar(neighbors.length);
      return center.sub(boid.position).multiplyScalar(0.01);
    };

    const getNeighbors = (boid, radius) => {
    ...
    const animate = () => {
      ...
      // Flock behavior changes with breath
      const cohesionStrength = 0.5 + breath * 1.5; // Tighter on inhale
      const separationStrength = 1.5 - breath * 0.8; // More spread on exhale
      const maxSpeed = 0.08 + breath * 0.04;
      const flockRadius = 2 + (1 - breath) * 2; // Larger flock spread on exhale

      // Touch creates attractor/repulsor
      let attractorX = 0, attractorY = 0, attractorActive = false;
      if (touchPointsRef.current.length > 0) {
        const touch = touchPointsRef.current.find(p => p.active);
        if (touch) {
          attractorX = (touch.x / window.innerWidth - 0.5) * 15;
          attractorY = -(touch.y / window.innerHeight - 0.5) * 12;
          attractorActive = true;
        }
      }

      boids.forEach(boid => {
        const neighbors = getNeighbors(boid, 1.5);

        // Apply rules
        const sep = separation(boid, neighbors).multiplyScalar(separationStrength);
        const ali = alignment(boid, neighbors);
        const coh = cohesion(boid, neighbors).multiplyScalar(cohesionStrength);

        boid.userData.velocity.add(sep).add(ali).add(coh);

        // Attractor/repulsor from touch
        if (attractorActive) {
          const toTouch = new THREE.Vector3(attractorX, attractorY, 0).sub(boid.position);
          const dist = toTouch.length();
          if (dist < 5) {
            toTouch.normalize().multiplyScalar(0.02 * (5 - dist) / 5);
            boid.userData.velocity.add(toTouch);
          }
        }

        // Keep flock centered (with breath-responsive radius)
        const toCenter = boid.position.clone().negate().multiplyScalar(0.001 / (flockRadius * 0.3));
        boid.userData.velocity.add(toCenter);

        // Limit speed
        if (boid.userData.velocity.length() > maxSpeed) {
          boid.userData.velocity.normalize().multiplyScalar(maxSpeed);
        }
        ...
        if (boid.userData.velocity.length() > 0.001) {
  ...
  return (
    ...
    >
      ...
      {(currentMode === 'geometry' || currentMode === 'jellyfish' || currentMode === 'flowerOfLife' || currentMode === 'mushrooms' || currentMode === 'tree' || currentMode === 'fern' || currentMode === 'dandelion' || currentMode === 'succulent' || currentMode === 'ripples' || currentMode === 'lungs' || currentMode === 'koiPond' || currentMode === 'lavaTouch' || currentMode === 'nebula' || currentMode === 'aurora' || currentMode === 'constellation' || currentMode === 'quantumFoam' || currentMode === 'neural' || currentMode === 'liquidMetal' || currentMode === 'orbitalRings' || currentMode === 'floatingIslands' || currentMode === 'mountains' || currentMode === 'cave' || currentMode === 'maloka' || currentMode === 'underwater' || currentMode === 'lotus' || currentMode === 'feather' || currentMode === 'candleFlame' || currentMode === 'incenseSmoke' || currentMode === 'hourglass' || currentMode === 'cymatics' || currentMode === 'inkWater' || currentMode === 'mycelium' || currentMode === 'treeLungs' || currentMode === 'sandMandala' || currentMode === 'murmuration') && (
